...
from .utils import simplex, solve_transportation_problem
import numpy as np
...
def transportation(request):
    if request.method == 'POST':
        try:
            ...
            cost_matrix = []
            num_sources = int(request.POST['num_sources'])
            num_destinations = int(request.POST['num_destinations'])
            for i in range(num_sources):
                row = [float(request.POST[f'cost_{i}_{j}']) for j in range(num_destinations)]
                cost_matrix.append(row)

            # Parse the supply and demand vectors
            supply = [float(request.POST[f'supply_{i}']) for i in range(num_sources)]
            demand = [float(request.POST[f'demand_{j}']) for j in range(num_destinations)]
